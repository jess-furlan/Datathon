# ================================================================================================
# - Problema: ranquear vagas por aderência ao perfil informado pelo hunter.
# - Sinais: (1) Texto TF‑IDF + Overlap de skills (Jaccard com sinônimos),
#           (2) Senioridade, (3) Idiomas (EN/ES), (4) Localização, (5) Contrato.
# - Combinação: média ponderada com pesos ajustáveis na UI (normalizados).
# - Foco: solução simples, interpretável e rápida de iterar (MVP).
# ================================================================================================
...
# =========================================
# Motivação: permitir importar/executar o arquivo em ambientes sem Streamlit
# (ex.: testes/CLI). Criamos um *shim* com no-ops e transformamos decorators
# de cache em identidades. Isso mantém a estrutura mas evita erros fora do runtime.
...
# =========================================
# Dependências centrais: numpy/pandas para manipulação vetorial/tabelas;
# scikit-learn para TF‑IDF e cosseno; unidecode para normalizar acentuação em PT-BR. Em ambiente sem essas libs, abortamos com mensagem amigável.
try:
    import numpy as np
    import pandas as pd
...
# Helpers • Linguagem
# =============================
# Pré‑processamento com foco em PT‑BR. Definimos stopwords, mapeamentos de senioridade e níveis de idioma, além de um léxico simples de sinônimos/termos
# relacionados para ampliar *recall* no match de skills.

# Stopwords PT‑BR: lista simples para reduzir ruído no TF‑IDF/tokenização. Mantemos minimalista para evitar remover termos técnicos curtos.
PT_STOP_WORDS = [
...
# Léxico de sinônimos/termos relacionados por skill.
# Serve para reconhecer equivalências (e.g., AWS ≈ EC2/S3/Lambda; SQL ≈ MySQL/Postgres).
# Mantido pequeno e editável para o domínio.
...
# norm_text(): Normaliza texto: minúsculas, remove acentos e espaços. Facilita matching e vetorização.
...
# tokenize(): Tokeniza texto em palavras/n-grams simples, filtrando stopwords e tokens muito curtos. Base para Jaccard.
...
# expand_query_skills(): Expande o conjunto de skills do candidato com sinônimos/termos relacionados (léxico simples). Aumenta recall.
...
# jaccard(): Calcula similaridade de Jaccard entre dois conjuntos de tokens (interseção/união).
...
# extract_lang_level(): Extrai/normaliza nível de idiomas (mapeia rótulos diversos para escala ordinal).
...
# extract_seniority(): Extrai/normaliza a senioridade a partir de texto livre.
...
# jobs_json_to_df(): Converte o JSON de vagas (dict) em DataFrame padronizado. Também monta campo textual “bag_text” com pesos.
def jobs_json_to_df(jobs: Dict[str, Any]) -> pd.DataFrame:
    ...
    for job_id, job in jobs.items():
        ...
        # Montamos um campo textual 'bag_text'
        # repetindo título/área para dar **mais peso**
        # a esses campos no TF‑IDF (heurística simples).
        bag_text = ' \n '.join([
            (str(titulo) + ' ') * 3,
            (str(area) + ' ') * 2,
            str(atividades),
...
# load_jobs(): Carrega vagas.json do disco e aplica jobs_json_to_df. Cacheado para evitar recomputação.
...
# save_candidate_record(): Salva dados do candidato em CSV simples (append). Garante diretório e cabeçalho.
...
# build_vectorizer(): Treina TfidfVectorizer no corpus das vagas. Parâmetros pensados para PT-BR e pequenas bases. Retorna também X (matriz TF-IDF).
@st.cache_resource(show_spinner=False)
def build_vectorizer(corpus: List[str]) -> Tuple[TfidfVectorizer, Any]:
    # Configuração do TF‑IDF:
    # - stop_words: usa nossa lista PT‑BR
    # - min_df=1: não descartamos termos raros (bases pequenas)
    # - max_df=0.97: remove termos super frequentes (quase stopwords)
    # - ngram_range=(1, 3): capta uni/bi/trigramas (títulos/skills compostos)
    # - sublinear_tf=True: atenua impacto de repetições
    # - smooth_idf=True e norm='l2': melhores práticas para cosseno
...
# text_similarity_score(): Calcula similaridade coseno entre candidato e vagas e reescala pelo p95 para robustez contra outliers.
def text_similarity_score(vect: TfidfVectorizer, X_jobs, candidate_text: str) -> np.ndarray:
    ...
    # Reescala por percentil 95 para evitar que outliers com
    # similaridade muito alta distorçam a distribuição.
    # Isso melhora a separação relativa entre vagas.
...
# level_match_score(): Score de aderência de idioma: compara nível exigido vs. informado, penalizando insuficiência.
...
# seniority_match_score(): Score de senioridade: aceita acima do requerido e penaliza defasagens com gradiente.
...
# location_match_score(): Score de localização: dá 0.6 para UF compatível e +0.4 para cidade. Pequena benevolência quando vazio.
...
# contract_match_score(): Score de tipo de contrato: 1.0 se texto contém, senão 0.6 (preferência fraca).
...
# skill_overlap_score(): Calcula overlap Jaccard entre tokens da vaga e tokens (com sinônimos) do candidato.
...
# compute_scores(): Combina os diversos sinais (texto, senioridade, idiomas, local, contrato) via pesos definidos no sidebar. Retorna DataFrame ordenado.
def compute_scores(df: pd.DataFrame, vect: TfidfVectorizer, X_jobs, *,
    ...
    # Combina dois sinais textuais: TF‑IDF (similaridade semântica)
    # + Overlap Jaccard de skills (ex.: AWS~EC2).
    # Pesos internos ajustáveis na UI (text_boost/skill_boost).
    ...
    # Agregação final: média ponderada dos componentes
    # (valores já em [0,1]). Pesos vêm do sidebar e são
...
# main_streamlit(): Função principal da UI Streamlit. Lê arquivo, coleta inputs, ajusta pesos, roda o matching e mostra resultados/diagnósticos.
def main_streamlit() -> None:
    ...
    with st.sidebar:
        ...
        # A Decision enviará um JSON com vagas.
        # Por simplicidade e para evitar erros do usuário,
        # forçamos o nome `vagas.json`.
        ...
        # Normalizamos os pesos informados pelo usuário
        # para somarem 1. Isso mantém a escala estável
        # e facilita comparar resultados entre execuções.
        ...
        # Mistura interna do componente de texto: o usuário escolhe o peso entre TF‑IDF (semântica)
        # e Overlap de skills (léxico).
    ...
    if trigger:
        ...
        with st.spinner('Calculando matches...'):
            ...
            # Concatenamos as informações do candidato num único
            # campo textual para o TF‑IDF (ex.: senioridade, idiomas,
            # skills e objetivo). Simples e eficaz para MVP.
            cand_text = " \n ".join([
                cand_senior,
            ...
            # Filtramos por um limiar ajustável para destacar apenas
            # vagas com score suficiente. A UI mostra top‑K sobre esse subconjunto.
        ...
        if matches.empty:
            ...
            # Se não houver match, registramos o candidato numa base CSV
            # para *pipelines* futuros. É uma persistência simples e barata.
        ...
        # Apresentamos uma visão tabular compacta para
        # priorização rápida pelo hunter.
        ...
        try:
            ...
            # Plot é opcional: se Matplotlib não estiver disponível
            # no ambiente, degradamos graciosamente e mostramos
...
# _default_weights(): Pesos padrão quando rodando em CLI/tests.
...
# run_tests(): Auto-testes simples para validar tokenização, sinônimos e ranking esperado.
def run_tests() -> None:
